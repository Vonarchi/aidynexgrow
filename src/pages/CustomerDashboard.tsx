...
const tabs = ['Overview', 'Application Status', 'Onboarding Checklist', 'Website Build Progress', 'Files and Assets', 'Messages', 'Revisions', 'Membership and Services', 'Billing', 'Support']
...
export function CustomerDashboard() {
  ...
  const nextAction = !application ? 'Start or submit your application' : !application.content_ready ? 'Prepare business content and onboarding assets' : currentStage === 'Client Review' ? 'Review the website preview' : currentStage === 'Revision' ? 'Watch for revision updates' : currentStage === 'Ready to Launch' ? 'Approve final launch details' : currentStage === 'Launched' ? 'Review membership and growth services' : 'Watch for the next project update'
  const checklist = [
    ...
    { label: 'Business Launch Membership confirmed', status: checklistStatus(Boolean(project?.hosting_status && project.hosting_status !== 'Not selected')), required: true, owner: 'Client', notes: project?.hosting_status || 'Confirm membership before launch.' },
  ...
  return <DashboardLayout title="Client Dashboard" subtitle="Track your application status, onboarding checklist, build progress, files, messages, revisions, membership, services, billing, and support.">
    ...
    {active === 'Membership and Services' && <div><div className="mb-6 rounded-3xl bg-navy-950 p-7 text-white"><ShoppingBag className="mb-3 text-gold-500" /><h3 className="text-2xl font-bold">Membership and Growth Services</h3><p className="mt-2 text-slate-300">Service requests are recorded now; Stripe-backed payment collection can be connected in a later phase.</p></div><div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">{snapshot.services.map((item) => <ServiceCard key={item.id} item={item} onRequest={handleService} />)}</div></div>}
    {active === 'Billing' && <EmptyState title="Billing placeholder" body="Membership, maintenance, subscription, and Stripe customer records will appear here once payments are connected." action={<button className="rounded-full bg-navy-950 px-5 py-3 font-semibold text-white">Request Billing Help</button>} />}
